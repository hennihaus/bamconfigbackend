...
import de.hennihaus.configurations.TaskConfiguration
...
class GithubMapperServiceImpl(@Property(DEFAULT_TITLE) private val defaultTitle: String) : GithubMapperService {

    override fun updateSchufaApi(api: SchufaApi, task: Task): SchufaApi {
        check(value = task.step == TaskConfiguration.SCHUFA_STEP) {
    ...
    override fun updateBankApi(api: BankApi, task: Task): BankApi {
        check(value = task.step == TaskConfiguration.SYNC_BANK_STEP) {
    ...
    private fun LinkedHashMap<String, SchufaResponse>.updateSchufaResponses(task: Task) = mapValuesTo(
        ...
        transform = { (key, response) ->
            task.responses.find { it.code == HttpStatusCode.fromValue(value = key.toInt()) }
                ?.let {
