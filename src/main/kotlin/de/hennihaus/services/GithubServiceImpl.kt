...
class GithubServiceImpl(
    ...
    override suspend fun updateOpenApi(task: Task) = when (task.step) {
        SCHUFA_STEP -> updateSchufa(task = task)
        SYNC_BANK_STEP -> updateBank(task = task)
        ASYNC_BANK_STEP -> {}
        else -> throw IllegalArgumentException(UNKNOWN_INTEGRATION_STEP_MESSAGE)
    }
    ...
    private suspend inline fun <reified T> getGithubFileAsEntity(fileConfig: GithubFileConfiguration): Pair<String, T> {
         ...
         */
        return Base64.getMimeDecoder().decode(file.content.toByteArray(charset = Charsets.US_ASCII)).let {
    ...
    companion object {
        const val UNKNOWN_INTEGRATION_STEP_MESSAGE = "Unknown integration step!"
    }
}
